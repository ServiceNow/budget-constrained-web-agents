...
class Evaluator:
    ...
    def __call__(self, info, client="gpt-3.5", version="naive"):
        ...
        if self.log_save_path:
            with open(self.log_save_path + "/outputs.json", "a") as f:
                f.write(
                    json.dumps(
                        {
                            "id": info["traj_name"],
            ...
            with open(f"{self.log_save_path}/{info['traj_name']}.md", "w") as md_file:
                ...
                md_file.write(f"## RM\n\n{eval_str}\n\n")
                md_file.write(f"## Final Response\n{info['response']}\n\n")
                ...
                if "config" in info:
                    ...
                    config_str = json.dumps(info["config"], indent=4)
    ...
    def eval_text(self, info, client):
        ...
        cap = info["captions"][-1] if info.get("captions") else ""
        prompt, sys_msg = build_text_eval_prompt(
            cap, info["intent"], response, action_history
        )
