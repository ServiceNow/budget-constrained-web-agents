...
def simplify_thought(step: str) -> str:
    ...
    if len(text_parts) > 0:
        ...
        response = llm_completion(
            ...
            messages=messages,
            temperature=0.0,
            usage_name=f"simplify_thought for task {args.result_dir}"
        )
        simplified_thought = get_contents(response, placeholder_if_failed="", remove_thinking="qwen" in args.model)[0]
